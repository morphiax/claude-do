...
**Domain research** — Before designing any non-trivial algorithm (scoring, ranking, matching, recommendation), researches how established products in the same domain solve the problem, what academic methods exist for the problem class, and what real users optimize for. The first algorithm invented from scratch is almost certainly worse than what the industry validated at scale. The research may change the problem framing, not just the parameters — "how to weight four factors" and "how should travel search rank results" are different problems with different answers. Prefers adapting established patterns over invention. Captures prior art and adaptation rationale in architecture.md and decisions.md.
