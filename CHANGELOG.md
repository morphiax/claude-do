...
- **Design: type/interface ownership**: When multiple roles share a data contract, one role is designated canonical owner to prevent duplicate definitions.
- **Execute: manual validation prompt**: Completion summary now includes a specific command for manual live-system verification when the goal involves external integrations.

## [3.0.3] - 2026-02-21

### Changed

- **Design and research: adversarial synthesis checkpoints**: Expert claim evaluation now uses three structured checkpoints (Could you be wrong?, Pre-mortem, Backward chaining) to surface concrete gaps before writing role briefs or recommendations. Actionable gaps are resolved immediately rather than deferred.
