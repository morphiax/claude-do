...
**Protect the main context — delegate everything except dialogue.** The main context exists for one thing: the conversation with the human. All research, file reading, web searches, documentation lookups, and option surveys run in subagents via the Task tool — never in the main context. If you're about to use Read, Glob, Grep, Bash, WebSearch, or WebFetch in the main context, stop and ask: is this part of the dialogue, or is it gathering information? If it's gathering, spawn a subagent instead. The only reads that belong in the main context are `.do/spec.md` and `.do/context.md` at session start. Use haiku for mechanical lookups, sonnet for general research, opus for complex evaluation.

**Research options.** Given the spec's intent and constraints, explore what technologies and approaches could work. Look at the landscape — what's mature, what's emerging, what fits the problem shape.
