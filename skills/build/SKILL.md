...
8. **Produce the plan.** Write the plan to the plan file. The plan starts with an **execution preamble**: the execution mechanism (create a task list via TaskCreate, execute each task in an isolated subagent via Task tool with `mode: "bypassPermissions"`), TDD workflow (write failing test first, run, implement minimum to pass, run again), coding conventions from context, quality gates, and any constraints that affect how tasks execute. The preamble must be self-sufficient — after plan approval, the SKILL.md may no longer be in scope. Each task is **self-contained**: test specification, implementation goal, relevant file paths and existing patterns, dependencies on other tasks, and any task-specific constraints. The plan assumes the executor has no prior context — only the plan itself. When the context defines quality conventions, the first task is setting up quality infrastructure.
...
10. **Execute tasks.** For each task in the approved plan, spawn a subagent (Task tool) with `mode: "bypassPermissions"`. Each subagent receives the execution preamble plus the task description — both derived from the approved plan. The plan IS the context. Each task follows TDD — write the failing test first, then the minimum implementation to pass it. Use haiku for mechanical work, sonnet for moderate implementation, opus for complex work.
...
- **The plan is the contract.** The plan is the complete execution contract — self-sufficient for agents with no prior context beyond the plan itself. Once approved, follow it. If execution reveals something the plan didn't anticipate — a spec ambiguity, a technical constraint, a mismatch — stop and flag it. Don't silently deviate from the approved plan.
