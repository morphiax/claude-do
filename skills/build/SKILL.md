...
**Protect the main context — delegate everything except orchestration.** The main context exists for two things: orchestrating the build and talking to the human. All implementation — writing code, creating config files, running tests, reading source files, researching APIs — runs in subagents via the Task tool. If you're about to use Read, Glob, Grep, Bash, Edit, or Write in the main context, stop and ask: is this orchestration, or is it implementation? If it's implementation, spawn a subagent instead. The only reads that belong in the main context are `.do/spec.md` and `.do/context.md` at session start, and task list management. Use haiku for mechanical work (config files, boilerplate), sonnet for moderate implementation (straightforward modules, test suites), opus for complex work (architectural code, nuanced logic).
