...
| **User context would materially change recommendations** | User context is evident from the conversation or project |

**User context check (mandatory before spawning researchers):** Before proceeding to Scope Assessment, verify you know enough about the user's situation to produce targeted recommendations. Key signals: Are they an end-user evaluating tools, or a developer building one? What device/platform/constraints do they have? What specific use case (not just the general domain)? If any of these would materially change the research scope or recommendation ranking, use `AskUserQuestion` to clarify. A broad landscape survey is the wrong default when the user has a specific situation — ask first, scope tight, then research.
...
3. **Compose the optimal researcher team for this specific research goal.** Default is 1 codebase-analyst + 1 external-researcher + 1 domain-specialist, but adapt roles to fit the clarified user context and topic. The goal is 3 complementary perspectives that cover the problem space — not 3 fixed roles applied regardless of context. For example: if there is no codebase to analyze, repurpose that slot for a perspective that IS needed (e.g., consumer-app evaluator, compliance researcher, practitioner-experience reviewer). Name each researcher descriptively and tailor their prompt to the actual research need. Always include at least one external/web researcher for grounding in prior art.
   - **Distinguish question type**: "What are the options?" (landscape survey) needs breadth — multiple methodology researchers. "How should I build X?" (integration question) needs depth — codebase + workflow-focused researchers, skip broad methodology surveys.
   - **Novel idea focus**: Identify the user's most novel or differentiating idea in their question. Ensure at least one researcher has this as a primary investigation focus — don't let novel ideas get diluted into sub-findings of broader topics. The novel-idea researcher must have at most 2 research tasks, with the novel idea consuming >50% of their effort. If there are 6+ research concerns, split the non-novel ones across the other researchers.
...
4. **Factual cross-check**: Before assembling, verify version numbers, project names, and quantitative claims from external/specialist researchers against codebase-analyst evidence. When researchers cite different versions of the same software (e.g., "v5" vs actual deployed "v6"), use the codebase-analyst's evidence as ground truth. Use the targeted fact-check exception (Read/Grep on a single file) to resolve discrepancies rather than propagating unverified claims into recommendations.
5. **Assemble sections**: Group findings by their section[] tags into the 5 knowledge sections. For each section, merge corroborating findings, surface contradictions, note gaps.
...
5. **Write recommendations**: Synthesize all sections into `recommendations[]`. Each recommendation has an `action` (adopt|adapt|investigate|defer|reject) and a concrete `designGoal` for `/do:design` (required when action is adopt or adapt). **Phased decomposition**: When a recommendation has 4+ sub-goals, the PRIMARY recommendation's designGoal must target the smallest independently shippable phase. The full vision goes into a separate `adopt` recommendation with `effort: "transformative"`. This prevents design from receiving an unmanageable mega-goal. **Preserve user framing**: When the user proposed specific terminology or concepts in their question, recommendations must explicitly validate, refine, or constructively challenge that terminology — not silently replace it with academic alternatives. The user's language is a signal of their mental model. **Audience targeting**: Lead with recommendations that match the clarified user context (e.g., consumer recommendations first if the user is an end-user, developer recommendations first if they're building). If recommendations span multiple audiences (end-user vs developer vs business), group and label them clearly so the user can immediately find what's relevant to THEIR situation.
