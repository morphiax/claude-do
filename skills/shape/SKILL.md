...
**Protect the main context — delegate everything except dialogue.** The main context exists for one thing: the conversation with the human. Everything else runs in subagents via the Task tool. Reading files, surveying code, researching patterns, checking git history — spawn a subagent. If you're about to use Read, Glob, Grep, Bash, or WebSearch in the main context, stop and ask: is this part of the dialogue, or is it gathering information? If it's gathering, spawn a subagent instead. The only reads that belong in the main context are `.do/spec.md` and `.do/context.md` at session start. Use haiku for mechanical reads, sonnet for moderate analysis, opus for complex interpretation.
